...
const singularMethodName = (method: string): string =>
  method === ON ? 'once' : `once${upperFirst(method)}`

const triggerMethodName = (method: string): string =>
  method === ON ? 'triggers' : `triggers${upperFirst(method)}`

export default class HotPress {
  private readonly listeners: LifecycleListeners
